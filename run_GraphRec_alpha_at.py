...
ABLATION STUDY: Item Attention (Alpha) Removal
This variant removes the item attention mechanism (alpha) for aggregating opinion-aware
interaction representations of items. It employs mean-based aggregation instead of
attention-based aggregation for item aggregation (modeling item-space user latent factors).
...
class UV_Aggregator_NoItemAttention(nn.Module):
    ...
    def forward(self, nodes, history_uv, history_r):
        ...
        for i in range(len(history_uv)):
            ...
            if self.uv == True:
                # User aggregation: use attention mechanism (alpha)
...
def main():
    ...
    print("="*80)
    print("ABLATION STUDY: Item Attention (Alpha) Removal")
    print("Using MEAN-based aggregation for item aggregation instead of attention")
    print("="*80)
    print(f"Used CUDA: {use_cuda}")

    embed_dim = args.embed_dim
    dir_data = './data/ciao_full'
